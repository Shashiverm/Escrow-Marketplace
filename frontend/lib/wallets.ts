  ...
  switch (type) {
    ...
    case "xbull": {
      if (window.xBullSDK) {
        ...
        const address = await (window as any).xbull.getPublicKey();
        return address;
      }
    ...
    case "lobstr": {
      ...
      // Fallback: Web LOBSTR connection flow / deep link simulation
    ...
    case "albedo": {
      ...
      // Fallback to Albedo URL intent pop-up protocol
      const width = 450;
      const height = 600;
...
export async function getLiveBalance(publicKey: string): Promise<number> {
  try {
    ...
    if (!res.ok) return 10000; // Default testnet balance fallback
